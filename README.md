...
Instead of inferring all the queries, we can configure them directly with the following parameters. All of them are mandatory except the one for creating final table which can be omitted if final table already exists.
  ...
  * impala.table.query.create: (not mandatory) query for creating final Impala table (applicable if final table does not exist).
...
Following parameters will not take effect.
  ...
  * impala.table.schema
  * impala.table.name
  * impala.table.parquet_file_size
